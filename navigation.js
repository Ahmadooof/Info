...
var navItems=Array.from(document.querySelectorAll('.bnav-item'));
...
if(currentPageEl)currentPageEl.textContent=String(index+1).padStart(2,'0');
}
...
slides.forEach(function(slide,i){if(i!==index)resetSlide(slide);});
...
slides.forEach(function(slide,i){if(i!==previous&&i!==index)resetSlide(slide);});
...
tl.to(prevSlide,{opacity:0,x:-direction*20,duration:0.18,ease:'power2.in'},0);
tl.to(nextSlide,{opacity:1,x:0,duration:0.28,ease:'power3.out'},0);
updateNav(current);
updatePageNumber(current);
}
function navigate(step){if(animating)return;goTo(current+step);}
...
else if(Math.abs(dy)>45&&Math.abs(dy)>Math.abs(dx)){navigate(dy>0?1:-1);}
},{passive:true});
...
if(!deleting){
charIndex++;
if(charIndex>role.length){deleting=true;setTimeout(type,1400);return;}
setTimeout(type,70);
}else{
charIndex--;
if(charIndex<0){
